...
echo "<form action='starter.php' method='post'><table border='1' width='700px' style='border-collapse:collapse;'><tr><td id='td1'>id</td><td id='td1'>Item Name</td><td id='td1'>Cost/item</td><td id='td1'>Select</td></tr>";
$query12="SELECT * FROM `starter`";
if($r=mysqli_query($conn,$query12))
{
	if(mysqli_num_rows($r)!=NULL)
	{
		...
		for($i=0;$i<sizeof($id)-1;$i++)
		{
			...
			echo "<td><a href='run2.php?del=".$c_id."&i=".$id[$i+1]."&type=starter'><input type='button' name='add' value='Add to Cart'></a></td></tr>";
		}
	}
	else
	{
